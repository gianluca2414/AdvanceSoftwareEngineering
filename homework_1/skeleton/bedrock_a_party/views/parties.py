...
from bedrock_a_party.classes.party import CannotPartyAloneError, Party, ItemAlreadyInsertedByUser, NotInvitedGuestError, NotExistingFoodError
...
def single_party(id):
    ...
    if 'GET' == request.method:
        ...
        result = _LOADED_PARTIES.get(id).serialize() #FORSE NON FUNZIONA

    elif 'DELETE' == request.method:
        # TODO: delete a party
        _LOADED_PARTIES.pop()
        result = jsonify({'Party deleted. Actual parties': len(_LOADED_PARTIES)})
...
def get_foodlist(id):
    ...
    if 'GET' == request.method:
        ...
        result = _LOADED_PARTIES.get(id).get_food_list().serialize()  #forse non fa e va fatto su due linee
...
def edit_foodlist(id, user, item):
    ...
    if 'POST' == request.method:
        ...
        try:
            result = _LOADED_PARTIES.get(id).add_to_food_list(item, user).serialize()
    ...
    if 'DELETE' == request.method:
        ...
        try:
            _LOADED_PARTIES.get(id).remove_from_food_list(item, user)
